...
import com.hyperlap.luckyhomefinder.crawlers.service.PropertyParser;
...
public class H28sePropertyFetcher implements PropertyParser {
	...
	public Property fetchProperty(String link) throws PropertyFetcherException {
		...
		try {
			...
			setPrice(trElements);
			setSiteId(link);
			setListedBy(trElements);
			setPropertyType(trElements);
			setFeets(trElements);
			setPostDate(trElements);
		} catch (IOException e) {
	...
	protected final void setLocation(Elements elements) {
		// Todo Implement set location function
	}

	protected final void setDistrict(Elements elements) {
		// Todo Implement set district function
	}
